...
public static class ShaderVariantCollectionExporter {
                ...
                foreach ( var sv in data ) {
                    ...
                    if ( shader != null ) {
                        ...
                        if ( EditorUtils.IsUnityDefaultResource( shaderPath ) ) {
                            ...
                            if (!shaderVariantsPath.StartsWith(ShaderVariantConst.DefaultProjectShaderPath))
                            {
                                shaderVariantsPath = shaderVariantsPath.Insert(0, ShaderVariantConst.UnityBuildInOrPackagePath);
                            }
    ...
    static void CollectAllBuildScenesShaderVariantsInScenes() {
        ...
        if ( scenes.Length > 0 ) {
            ...
            for ( int i = 0; i < scenes.Length; ++i ) {
                var scene = scenes[ i ];
                var path = scene.path;
                if ( path == ShaderVariantConst.BlackholeScenePath || !File.Exists( path ) ) {
                    continue;
                }
                taskList.Add(
                ...
                taskList.Add(
                    ...
                    () => {
                        ...
                        EditorSceneManager.OpenScene(ShaderVariantConst.BlackholeScenePath );
                    }
                );
            }
            taskList.Add(
    ...
    static bool SortShaderVariantCollection( ShaderVariantCollection svc ) {
        var data = ShaderVariantCollectionHelper.ExtractData( svc );
        bool isDirty = false;
        ...
        for ( int i = 0; i < shaderNames.Count; ++i ) {
            ...
            for ( int j = 0; j < svList.Count; ++j ) {
                ...
                if (vc.keywords.Length == 0)
                {
                    continue;
                }

                if (vc.keywords.Length == 1 && string.IsNullOrEmpty(vc.keywords[0]))
                {
                    continue;
                }
    ...
    static bool SaveShaderVariantCollection( String assetPath, SVCWithList newSvc ) {
        ...
        if ( curSvc == null ) {
            bool isDirty = SortShaderVariantCollection( newSvc.svc );
            if (!isDirty)
            {
                Debug.LogWarningFormat("Ignore create new ShaderVariantCollection: {0}, shaderCount: {1}, variantCount = {2}",
                    assetPath, newSvc.svc.shaderCount, newSvc.svc.variantCount);
                return false;
            }
    ...
    internal class ShaderVariantValidationCache {
        ...
        internal void Clear() {
            ...
            try {
                if ( File.Exists(ShaderVariantConst.CacheDataFilePath) ) {
                    File.Delete(ShaderVariantConst.CacheDataFilePath);
                    Debug.LogFormat( "Remove: {0}", ShaderVariantConst.CacheDataFilePath);
                }
            } catch ( Exception e ) {
        ...
        internal void Reload() {
            try {
                ...
                var jo = JSONObject.Create( File.ReadAllText(ShaderVariantConst.CacheDataFilePath) );
        ...
        internal void Save() {
            try {
                ...
                FileUtils.CreateDirectory(ShaderVariantConst.CacheDataFilePath);
                File.WriteAllText(ShaderVariantConst.CacheDataFilePath, root.Print( true ) );
            } catch ( Exception e ) {
                UDebug.LogException( e );
            }
        }
    ...
    static void SaveShaderVariants( bool keepTempShaderVariants = true, bool refreshAll = true,
        ...
        var tempPath = ShaderVariantConst.TempShaderVarientFilePath;
        try {
...
#if UNITY_EDITOR
        var collection = AssetDatabase.LoadAssetAtPath<GameObject>(ShaderVariantConst.ShaderCollectionPrefabPath);
        if ( collection != null ) {
            var isNew = false;
    ...
    static IEnumerator ShaderVariantsCollector( bool includeScenes, List<String> _excludeShaders, List<String> _updateOnlyShaders, bool debugBypass = false, bool useShaderVariantValidationCache = true ) {
        ...
        var realtimeScenePath = ShaderVariantConst.RealtimeScenePath;

        var lightEnvPathList = new List<string>(ShaderVariantConst.LightEnvPathList);
